...
pub fn start_etcd_thread(etcd_urls: &str, cluster_name: &str, uuid: &Uuid, host: &str, port: usize) {
...
pub async fn etcd_get_executors(etcd_urls: &str, cluster_name: &str) -> Result<Vec<ExecutorMeta>> {
    match Client::connect([etcd_urls], None).await {
        ...
        Err(e) => Err(ballista_error(&format!("Failed to connect to etcd {:?}", e.to_string()))),
    }
}

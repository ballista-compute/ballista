...
use crate::{memory_stream::MemoryStream, utils::write_stream_to_disk};
...
impl ShuffleWriteExec {
    ...
    pub fn new(child: Arc<dyn ExecutionPlan>, child_partition: usize, output_path: &str, target_partition_count: usize) -> Self {
...
impl ExecutionPlan for ShuffleWriteExec {
    ...
    async fn execute(&self, partition: usize) -> Result<SendableRecordBatchStream> {
        ...
        let mut partition_location = StringBuilder::with_capacity(num_partitions, num_partitions * 256);
...
mod tests {
    ...
    async fn test() -> Result<()> {
        ...
        let shuffle_write_exec = ShuffleWriteExec::new(mem_table, 0, tmp_dir.path().to_str().unwrap(), 200);

        assert_eq!(1, shuffle_write_exec.output_partitioning().partition_count());
